    ...
    function SubmitBlock(blockHex, finishedCallback){
        ...
        _this.daemon.cmd(rpcCommand,
            ...
            function(results){
                for (var i = 0; i < results.length; i++){
                    ...
                    if (result.error) {
                        emitErrorLog('rpc error with daemon instance ' +
                                result.instance.index + ' when submitting block with ' + rpcCommand + ' hex = ' + blockHex + ', ' +
                ...
                emitLog('Submitted Block using ' + rpcCommand + ' successfully to daemon instance(s), hex = ' + blockHex);
                finishedCallback();
            }
        );
