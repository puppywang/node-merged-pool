...
var JobManager = module.exports = function JobManager(options){
 ...
 //public members
    ...
    this.processTemplate = function(rpcData){
        ...
        console.log("FUCK: option = " + JSON.stringify(options));
    ...
    this.processShare = function(jobId, previousDifficulty, difficulty, extraNonce1, extraNonce2, nTime, nonce, ipAddress, port, workerName){
        ...
        var headerBuffer = job.serializeHeader(merkleRoot, nTime, nonce, false);
        var headerBufferForHash = job.serializeHeader(merkleRoot, nTime, nonce, true);
        var headerHash = hashDigest(headerBufferForHash, nTimeInt);
        ...
        blockHashInvalid = blockHasher(headerBufferForHash, nTime).toString('hex');

        //Check if share is a block candidate (matched network difficulty)
        if (job.target.ge(headerBigNum)){
            blockHex = job.serializeBlock(headerBuffer, coinbaseBuffer).toString('hex');
            blockHash = blockHasher(headerBufferForHash, nTime).toString('hex');
            //blockHash = util.reverseBuffer(util.sha256d(headerBufferForHash, nTime)).toString('hex');
        }
        else {
