...
**/
var BlockTemplate = module.exports = function BlockTemplate(jobId, rpcData, poolAddressScript, extraNoncePlaceholder, reward, txMessages, recipients, auxMerkleTree, isBCA){
    ...
    this.serializeHeader = function(merkleRoot, nTime, nonce, forHash){
        // We prefer old format.
        var isBCAFormat = false; /*isBCA && !forHash;*/
        var header =  new Buffer(isBCAFormat ? 84 : 80);
        var position = 0;
        if (isBCAFormat) {
            // New block format flag
            header.writeUInt32BE(0x80000000, position);
            position += 4;
        }
